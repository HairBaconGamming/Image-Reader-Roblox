...
function jpeg.new(buffer, printDebugInformation, printProgress, preventTimeouts)
	local rawInputBytes = {}
	for i = 1, string.len(buffer) do
		rawInputBytes[i] = string.byte(string.sub(buffer, i, i))
	end
	-- Verify it's a JPEG
	...
	local function SOFn(marker, pos, bytes)
		...
		jpeg.Width = samplesPerLine
		jpeg.Height = lineNumber
		jpeg.Precision = precision
		jpeg.ComponentCount = componentCount
		jpeg.FrameType = marker

		jpeg.MCUCountX = math.floor((jpeg.Width - 1) / 8)
		jpeg.MCUCountY = math.floor((jpeg.Height - 1) / 8)

		-- Fill in image with blank data
		for x = 1, jpeg.Width do
			jpeg.ImageData[x] = {}

			for y = 1, jpeg.Height do
		...
		if printDebugInformation then
			print("Image Resolution: " .. jpeg.Width .. "x" .. jpeg.Height)
	...
	local function SOS(marker, pos, bytes)
		...
		local mcuCountX = math.floor((jpeg.Width - 1) / 8)
		local mcuCountY = math.floor((jpeg.Height - 1) / 8)
					...
					for h = 0, componentHorizontalSample - 1 do
						...
						for i = 0, iLimit do
							for j = 0, jLimit do
								...
								if pixelX <= jpeg.Width and pixelY <= jpeg.Height then
